...
@include('common::includes.datatable')
  ...
  {{-- <script src="{{asset('')}}js/sliders/script.js"></script> --}}

    @if(session('updated'))
        <script>
            Swal.fire({
                title: 'أحسنت!',
                text: 'لقد تم تعديل الصورة بنجاح',
                icon: 'success',
                customClass: {
    ...
    @if(session('created'))
        <script>
            Swal.fire({
                title: 'أحسنت!',
                text: 'لقد تم انشاء الصورة بنجاح',
                icon: 'success',
                customClass: {
                    confirmButton: 'btn btn-primary'
                },
        ...
        var dt_basic = dt_basic_table.DataTable({
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                        var $user_img = full['image'],
                            $name = data;
                        if ($user_img) {
                             // $user_img = window.location.origin+'/uploads/slider/'+$user_img;
                            // For Avatar image
                            var $output =
                ...
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                        var activation = '';
                        if(full['is_active'] ==0) activation ='مفعل'; else activation = 'الغاء التفعيل';
                        return (
                            '<div class="d-inline-flex">' +
                            '<a class="pe-1 dropdown-toggle hide-arrow text-primary" data-bs-toggle="dropdown">' +
                            feather.icons['more-vertical'].toSvg({ class: 'font-small-4' }) +
                            '</a>' +
                            '<div class="dropdown-menu dropdown-menu-end">'@can('Edit-slider') +
                            '<a href="sliders/activate/'+data+'" class="dropdown-item">' +
                            ...
                            '</div>'
                            @can('Edit-slider') +
                            '<a href="sliders/'+data+'/edit" class="item-edit">' +
            ...
            buttons: [
                ...
                {
                    text: feather.icons['plus'].toSvg({ class: 'me-50 font-small-4' }) + 'اضافة',
                    ...
                    action: function (e, dt, node, config)
                    {
                        ...
                        window.location.href = './sliders/create';
                    },
                    init: function (api, node, config) {
                        $(node).removeClass('btn-secondary');
                    }
    ...
    $('.datatables-basic tbody').on('click', '.delete-record', function () {
        ...
        Swal.fire({
            ...
            if (result.value) {
                ...
                $.ajax(
                    {
                        url: window.location.origin+"/admin/sliders/"+id,
                        type: 'POST',
                        data: {
                            "id": id,
                            "_method": "DELETE",
